# Innoket Cold + Hot wallet MasterNode setup guide
...
* Static IP Address
* Port 37658 port forwarded from your router to your Ubuntu server
* Basic Linux skills
...
This is the wallet where the MasterNode collateral of 10.000 IKT coins will have to be transferred and stored.
After the setup is complete, this wallet doesn't have to run 24/7 and will be the one receiving the rewards.

### 1. Install and open the Innoket-Qt wallet on your machine.

#### i.    Download the newest innoket-{version}-{OS}.zip wallet from https://github.com/innoket/innoket/releases
#### ii.   Extract the innoket-qt.exe from innoket-{version}-{OS}.zip (e.g. for Windows)
#### iii.  Start the new innoket-qt.exe
...
#### viii. Encrypt your wallet with a long passphrase and either save it in a password manager such as keepass, or write it down and keep it safe (in a locked compartment or safe) (recommended). This passphrase is your only key to your wallet, do NOT lose it or you will lose all your IKT. Do not let anyone steal your passcode or wallet either, just like in real life!
To encrypt the wallet, go to Settings > Encrypt wallet. Enter the passphrase, click ok. You will then have to restart the wallet and then go to Settings > Unlock Wallet and then enter the passphrase to unlock the wallet, for staking, controlling the masternode or sending your IKT.
#### ix.   Back up your wallet.dat in case of a mistake as soon as you encrypt your wallet. Once you have encrypted the wallet, your previous backups will not work, so back it up by going to File > Backup Wallet and save the backup to more than one place. Such as a USB key or a network share. I recommend keeping your wallet and passphrase seperate so that no one can steal both. They would need both to do anything to your wallet or IKT!
...
### 4. Send exactly 10.000 IKT coins to the address you just copied. Double check you've got the correct address before transferring the funds.
...
>If you have given the transaction a few minutes to get a confirmation and you still do not get any output from masternode outputs it means that you have not sent EXACTLY 10.000 IKT in a single transaction. 
...
```
MN01 your_vps_ip_address:37658 your_masternode_key_output_from-masternode_genkey txhash_from-masternode_outputs Outputidx_from-masternode_outputs
```
...
```
MN01 12.34.23.123:37658 7UK09x1WWANK63WnxaqSxLijVjLkeXnQ9aZjLPex6cMTEREn3LP 9fe8534b7900108a746d9bca944e6182996d00a081abe9ee09890aaff471a0dfd 0
```
...
sudo ufw allow 37658/tcp
sudo ufw logging on
sudo ufw --force enable
```

If you are running the MasterNode server in Amazon AWS or another place where additional firewalls are in place, you need to allow incoming connections on port 37658/TCP



### 5. Install the Innoket CLI wallet. Always download the latest [release available](https://github.com/innoket/innoket/releases). The file will be innoket.zip. Next, unpack it

If you are already running a `innoketd` on your server and want to upgrade it, stop the current one with:
```
innoketd stop
```
Run the following command until the innoketd process disappears.
```
ps aux | grep innoketd | grep -v grep
```
...
wget https://github.com/innoket/innoket/releases/download/v2.1.0/innoket-2.1.0-x86_64-linux-gnu.tar.gz
tar -zxvf innoket-2.1.0-x86_64-linux-gnu.tar.gz
rm innoket-2.1.0-x86_64-linux-gnu.tar.gz
chmod +x ./innoket-2.1.0/bin/innoketd
sudo mv ./innoket-2.1.0/bin/innoketd /usr/local/bin
chmod +x ./innoket-2.1.0/bin/innoket-cli
sudo mv ./innoket-2.1.0/bin/innoket-cli /usr/local/bin
cd ~
innoketd
```

You'll get a start error like `Error: To use innoketd, or the -server option to innoket-qt, you must set an rpcpassword in the configuration file`. It's expected because we haven't created the config file yet.

The service will only start for a second and create the initial data directory(`~/.innoket/`).

### 6. Edit the MasterNode main wallet configuration file:
```
nano ~/.innoket/innoket.conf
```
...
maxconnections=250
masternode=1
externalip=<ip_address_here>:37658
masternodeaddr=<ip_address_here>:37658
...
maxconnections=250
masternode=1
externalip=12.34.23.123:37658
masternodeaddr=12.34.23.123:37658
...
The masternode debug log (`~/.innoket/debug.log`) will contain this line on a successful activation:
```
...
IKT - AVhZr3Sr96fVWCvKSqd6kGPWEQB9U89LUy
