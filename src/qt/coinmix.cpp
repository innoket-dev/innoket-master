...
class TxViewDelegate2 : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate2() : QAbstractItemDelegate(), unit(BitcoinUnits::IKT)
    {
    }
...
void Coinmix::updateDarksendProgress()
{
    ...
    QString strAnonymizeIKTAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeIKTAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeIKTAmount = strAnonymizeIKTAmount.remove(strAnonymizeIKTAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeIKTAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeIKTAmount * COIN) nMaxToAnonymize = nAnonymizeIKTAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeIKTAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeIKTAmount));
        strAnonymizeIKTAmount = strAnonymizeIKTAmount.remove(strAnonymizeIKTAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeIKTAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
